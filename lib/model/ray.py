...
def sigma_density_function(sdf, s):
    s = torch.tensor(s, device=sdf.device, dtype=sdf.dtype)
    # s = torch.clamp(s, min=1.0)
    return torch.exp( torch.log(s) - s*sdf - 2*( torch.log(1 + torch.exp( -s *sdf ))) ).clip(0,1)
