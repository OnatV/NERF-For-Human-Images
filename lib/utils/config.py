...
def parse_options():
    ...
    train_group.add_argument('--use_eikonal_loss', type=bool, default=False)

    ###################
    # Arguments for Point Sampling
    ###################
    sample_group = parser.add_argument_group('sampling')
    sample_group.add_argument('--num-pts-per-ray-c', type=int, default=64,
                                help='Number of points to sample per ray for coarse network')
    sample_group.add_argument('--num-pts-per-ray-f', type=int, default=128,
                                help='Number of points to sample per ray for fine network')
    sample_group.add_argument('--near', type=float, default=1.0,
                                help='Near plane')
    sample_group.add_argument('--far', type=float, default=3.0,
                                help='Far plane')
    sample_group.add_argument('--hierarchical', type=bool, default=True,
                                help='Usage of hierarchical volume rendering')
    
    ###################
    # Arguments for model
    ###################
    net_group = parser.add_argument_group('network')
    net_group.add_argument('--network-type', type=str, default='skeleton', help  = 'Network type to choose from [skeleton, updated_skeleton, double_mlp]')
    net_group.add_argument('-sigma-out-layer', type=int, default=4)
    net_group.add_argument('--out-dim', type=int, default=4,
                          help='output dimension')
    net_group.add_argument('--num-layers', type=int, default=6, 
