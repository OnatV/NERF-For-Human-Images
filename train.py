...
from lib.model.mlps import MLP, get_activation_class, MLP2, DoubleMLP, MLP3
...
def main(config):
    ...
    if config.network_type == 'skeleton':
        network = MLP(pe.out_dim, config.out_dim, activation=get_activation_class(config.activation), 
                    ...
                    skip=[config.skip] )
    elif config.network_type == 'updated_skeleton':
        network = MLP2(pe.out_dim, output_dim= 3, activation=get_activation_class(config.activation),
                       num_layers= config.num_layers , hidden_dim=config.hidden_dim,
                       skip=[config.skip], sigma_dim=1, sigma_out_layer=config.sigma_out_layer)
    elif config.network_type == 'updated_skeleton2':
        network = MLP3(pe.out_dim, output_dim= 3, activation=get_activation_class(config.activation),
                       num_layers= config.num_layers , hidden_dim=config.hidden_dim,
                       skip=[config.skip], sigma_dim=1, sigma_out_layer=config.sigma_out_layer)

    elif config.network_type == 'double_mlp':

        network = DoubleMLP(input_dim = pe.out_dim, output_dim= 3,
                       num_layers= config.num_layers , hidden_dim=config.hidden_dim,
                       skip=[config.skip], sigma_dim=257, sigma_out_layer=config.sigma_out_layer)
